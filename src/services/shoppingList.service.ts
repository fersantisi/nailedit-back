...
export const addNewStock = async (id: number): Promise<void> => {
  try {
    ...
    // Remove the item from shopping list after successfully adding to stock
    await deleteItem(id);
  } catch (error) {
    if (error instanceof Error) {
      console.log(error);
...
export const getShoppingListPdf = async (userId: number): Promise<Buffer> => {
